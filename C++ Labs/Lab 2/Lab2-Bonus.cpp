...
void spoolBackorder(double stock,double order,double shiprate)
{
	...
	else if(order > stock)
	{
		...
		cout<<" "<<endl;
		std::cout<<" "<<left<<setw(10)<<"Back Order Charges"<<left<<setw(10)<<endl;
		std::cout<<"--------------------"<<left<<setw(20)<<endl;
		double sellingprice3 = (backorder)*(100);
		std::cout<<"Selling Price Of Items on BackOrder: "<<"$"<<std::setprecision(2)<<std::fixed<<sellingprice3<<endl;
		std::cout<<"Total of BackOrder: "<<"$"<<std::setprecision(2)<<std::fixed<<sellingprice3<<endl;
...
int main()
{
	...
	cout << "BackOrder Will by shipped on: " << dt << endl;
